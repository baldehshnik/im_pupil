...
fun UpdateDialog(
    ...
    codeInitial: String,
    onSave: (UpdateGroupMemberModel) -> Unit
) {
    var firstname by remember { mutableStateOf(firstnameInitial) }
    var lastname by remember { mutableStateOf(lastnameInitial) }
    var patronymic by remember { mutableStateOf(patronymicInitial) }
    var code by remember { mutableStateOf(codeInitial) }
        ...
        confirmButton = {
            TextButton(onClick = {
                if (firstname.length < 2) {
                    ...
                    val model = UpdateGroupMemberModel(idInitial, firstname, lastname, patronymic, code)
                    onSave(model)
                    onDismiss()
                }
            }) {
